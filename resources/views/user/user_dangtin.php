...
// --- BẢO VỆ TRANG: NẾU CHƯA ĐĂNG NHẬP, CHUYỂN VỀ TRANG ĐĂNG NHẬP ---
if (!isset($_SESSION['user_id'])) {
    // Lưu lại thông báo để hiển thị sau khi chuyển hướng (tùy chọn)
    // $_SESSION['login_required_message'] = "Vui lòng đăng nhập để đăng tin.";
    header("Location: ../../../app/Controllers/dangnhap.php");
    exit();
}
...
if(isset($_POST['btn_dangtin'])) {
    // Sử dụng real_escape_string để chống SQL Injection
    $title = $conn->real_escape_string($_POST['title']);
    $price = $conn->real_escape_string($_POST['price']);
    $desc = $conn->real_escape_string($_POST['description']);
    $cate_id = $conn->real_escape_string($_POST['category_id']);
    ...
    if(move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
        ...
        $stmt = $conn->prepare("INSERT INTO products (seller_id, category_id, title, description, price, status) VALUES (?, ?, ?, ?, ?, 'pending')");
        $stmt->bind_param("iisds", $seller_id, $cate_id, $title, $desc, $price);
        ...
        if($stmt->execute()) {
            ...
            $stmt_image = $conn->prepare("INSERT INTO product_images (product_id, filename, sort_order) VALUES (?, ?, 0)");
            $stmt_image->bind_param("is", $product_id, $db_image_path);
            $stmt_image->execute();
            
            echo "<script>alert('Đăng tin thành công! Tin của bạn đang chờ duyệt.'); window.location.href='user_dashboard.php';</script>";
            exit();
...
<div class="sidebar">
    ...
    <?php if (isset($_SESSION['user_id'])): ?>
        <a href="../../../app/Controllers/dangxuat.php" class="text-danger mt-5"><i class="fa-solid fa-right-from-bracket me-2"></i> Đăng Xuất</a>
    <?php endif; ?>
</div>

<div class="main-content">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h4 class="mb-0 text-warning"><i class="fa-solid fa-camera"></i> Đăng Bán Sản Phẩm</h4>
        <?php
            if (!isset($_SESSION['user_id'])) {
                echo '<a href="../../../app/Controllers/dangnhap.php" class="btn btn-danger">Đăng Nhập</a>';
            }
        ?>
    </div>
